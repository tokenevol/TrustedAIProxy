...
🌐 **项目网站：[tokenevol.github.io/TrustedAIProxy](https://tokenevol.github.io/TrustedAIProxy/zh/)**
...
推荐在所需的 Confidential Space 环境中将 TrustedAIProxy 作为 sidecar 或独立的内部服务运行，只允许中转站访问其代理端口。
...
仓库统一使用 Python 参考客户端完成端到端验签。它通过正常的客户服务地址发起 OpenAI Chat Completions 请求，并在验签成功后把 nonce 写入本地 SQLite 防重放缓存。传入的公钥必须已完成运行环境证明验证：
...
Google attestation 不需要每次请求都调用。用户可以按 workload 启动周期或会话验证环境证明，同时遵守证明有效期和本地策略，再通过 `X-Attestation-Proof-Ref` 关联每条业务响应。
...
产品网站位于 [`website/`](website/)，是无需构建步骤的静态站点。默认展示英文，主页、部署指南和用户指南均提供位于 [`website/zh/`](website/zh/) 的中文版本。页头语言按钮切换到同一页面的另一语言，站内导航保持所选语言；启用 JavaScript 时也会保留当前章节锚点。[`.github/workflows/pages.yml`](.github/workflows/pages.yml) 会在 `main` 分支的站点文件或 Pages workflow 发生变化时自动发布，也可以从 GitHub Actions 页面手动触发。
